...
impl<V> SelectValue<V> {
    /// Create new select item value.
    pub fn new<S: Into<String>>(value: V, text: S) -> Self {
        Self {
            value,
...
impl<V: std::fmt::Display> SelectValue<V> {
    /// Create new select item value.
    ///
    /// `text_value` generated by [Display](std::fmt::Display) implementation.
    pub fn new_display(value: V) -> Self {
        Self {
...
pub(super) struct SelectContext<T: Clone + PartialEq + 'static> {
    ...
    pub cursor: Memo<Option<SelectValue<T>>>,
    /// Set the value callback
    pub set_value: Callback<Option<SelectValue<T>>>,
    /// A list of options with their states
    pub options: Signal<Vec<OptionState<SelectValue<T>>>>,
