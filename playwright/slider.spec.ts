...
test('test', async ({ page }) => {
  ...
  // The initial aria-valuenow should be 0
  await expect(thumb).toHaveAttribute('aria-valuenow', '0');
  await thumb.focus();
  // The aria-valuenow should be 10 after pressing Shift+ArrowRight
  await page.keyboard.press('Shift+ArrowRight');
  await expect(thumb).toHaveAttribute('aria-valuenow', '10');
  await page.keyboard.press('Shift+ArrowRight');
  // The aria-valuenow should be 20 after pressing Shift+ArrowRight again
